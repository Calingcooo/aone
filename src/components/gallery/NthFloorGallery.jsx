  ...
  return (
    <div className="col-span-5 relative mt-5 md:my-10">
      <div className="flex justify-center items-center mb-5">
        <LazyLoadImage
          src={floorSliderData.image}
          alt="Image Selected"
          className="flex justify-center h-full w-4/5 shadow-drop-lg rounded-xl"
        />
      </div>
      <div className="relative flex items-center">
        <FontAwesomeIcon
          icon={faPlay}
          className="rotate-180 cursor-pointer h-10 md:h-14 opacity-75 hover:opacity-100"
          id="arrow"
          onClick={slideLeft}
        />
        <div
        ...
        >
          {NthFloor.map((img, i) => (
            <LazyLoadImage
              ...
              key={img.id}
              alt={img.name}
              className="max-h-14 md:max-h-24 w-30 cursor-pointer rounded-xl p-2 drop-shadow-xl hover:scale-105 ease-in-out duration-300"
        ...
        <FontAwesomeIcon
          icon={faPlay}
          className="cursor-pointer h-10 md:h-14 opacity-75 hover:opacity-100"
          id="arrow"
          onClick={slideRight}
        />
      </div>
